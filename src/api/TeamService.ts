...
export class TeamService {
  ...
  async updateTeamInstructorInfo(
    ...
    try {
      const accessToken = sessionStorage.getItem("accesstoken");
      const response = await apiClient().patch(`${url}/update/member`, data, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  async InviteInstructor(data: InviteRequestDTO): Promise<void> {
    try {
      const accessToken = sessionStorage.getItem("accesstoken");
      const response = await apiClient().post("/notification/invite", data, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  async cancelTeamInvite(data: InviteCancelRequestDTO): Promise<void> {
    try {
      const accessToken = sessionStorage.getItem("accesstoken");
      const response = await apiClient().post("/notification/cancel", data, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
