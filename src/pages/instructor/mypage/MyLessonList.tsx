    ...
    return (
        <div className="w-screen min-w-screen">
            {innerWidth > 640 ? (
                ...
                <NavbarInstructorMobile />
            )}
            <div className="flex justify-center mt-20 text-3xl font-bold mb-10">
                강습 내역
            </div>
            <div className="flex justify-center ">
                <div className="bg-primary-50 w-[350px] sm:w-[1000px] min-w-96 rounded p-8">
                    <LessonSection
                        title="진행 예정"
                        lessons={lessons.filter(
                            (lesson) => lesson.lessonStatus === "notStart"
                        )}
                    />
                    <LessonSection
                        title="피드백 미작성"
                        lessons={lessons.filter(
                            (lesson) => lesson.lessonStatus === "lessonFinished"
                        )}
                    />
                    <LessonSection
                        title="피드백 작성"
                        lessons={lessons.filter(
                            (lesson) => lesson.lessonStatus === "yesFeedback"
                        )}
                        reviews={reviews}
                    />
                </div>
