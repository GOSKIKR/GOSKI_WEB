...
const FeedbackEdit: React.FC = () => {
    ...
    useEffect(() => {
        const fetchFeedback = async () => {
            const feedback = await userFeedbackService.getUserFeedback(state.lessonId);
            if (feedback) {
    ...
    const updateFeedback = async () => {
        ...
        deleteVideoFiles.forEach((id) => {
            formData.append("deleteVideos", id.toString());
        });

        deleteImageFiles.forEach((id) => {
            formData.append("deleteImages", id.toString());
        });

        await feedbackService.updateFeedback(feedback?.feedbackId, formData);
    ...
    return (
        <div>
            ...
            <div className="flex justify-center mt-20 text-3xl font-bold mb-10">피드백 수정</div>
            ...
            <div className="flex justify-center my-10">
                <button className="bg-primary-700 text-white m-2 px-4 py-2 rounded" onClick={updateFeedback}>
                    수정완료
                </button>
                <button className="bg-primary-900 text-white m-2 px-4 py-2 rounded" onClick={() => navigate("/instructor/my-lesson")}>
                    돌아가기
                </button>
            </div>
        </div>
